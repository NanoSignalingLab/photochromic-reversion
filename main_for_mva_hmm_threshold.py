...
if __name__ == '__main__':
    ...
    def calulate_hmm_precison_with_simulating_tracks( f1,min_track_length, dt, plotting_flag, plotting_saving_nice_image_flag,tracks_saving_flag,
        ...
        for index, row in df_values[:2].iterrows():
            ...
            deep_df3=computing_distance_wrapper(deep_df2, threshold_dist)
            deep_df4=calculate_angles_wrapper(deep_df3, threshold_cont_angle)
            deep_df5=calculate_KDE_wrapper(lys_x, lys_y, deep_df4, threshold_cont_KDE)
            deep_df6=calculate_intersections_wrapper(lys_x, lys_y, deep_df5, threshold_cont_intersections)
            ...
            if index==0:
                ...
                threshold_list = [[threshold_dist, threshold_cont_angle, threshold_cont_KDE, threshold_cont_intersections, D]]
            else:
                list_final_accuracy.append(list_accuracy)
                threshold_list.append([threshold_dist, threshold_cont_angle, threshold_cont_KDE, threshold_cont_intersections, D])
            
        
        df_final_accuracy=pd.DataFrame(list_final_accuracy, columns=["percent_both_confined", "percent_both_unconfined","percent_correct","percent_correct_confined","percent_correct_unconfined","percent_sim_confined","percent_sim_unconfined", "precision_confined",  "precision_unconfined","recall_confined", "recall_unconfined",  "fbeta_confined","fbeta_confined","fbeta_confined", "support_confined", "support_unconfined", "logD_mean_diff", "logD_mean_cluster_diff", "mean_clusters_per_track", "total_time_in_cluster_per_track", "mean_time_in_clusters_per_track", "mean_clustered_points", 'tpr', 'fpr'])
        print(threshold_list)
        df_treshold_values = pd.DataFrame(threshold_list, columns=['threshold_dist', 'threshold_cont_angle', 'threshold_cont_KDE', 'threshold_cont_intersections', 'D'])
        print(df_treshold_values)
    ...
    def make_deep_df(df, min_track_length):
        ...
        for i in grouped["TRACK_ID"].unique():
            ...
            if s.shape[0]>min_track_length: # parameter to set minimun length of track duration (eg. 25 time points)
                count2+=1
    ...
    def consecutive(col, seg_len, threshold, deep_df): # col= string of cl indf, seg_len=segment length of consecutive
    ...
    def computing_distance_wrapper(deep_df, threshold_dist):
    ...
    def calculate_angles_wrapper(deep_df, threshold_cont_angle):
        n=deep_df["pos_t"]
        x=deep_df["pos_x"]
        y=deep_df["pos_y"]
        ...
        angle_cont_lys=consecutive("angles", 10, threshold_cont_angle, deep_df)
    ...
    def calculate_KDE_wrapper(lys_x, lys_y, deep_df, threshold_cont_KDE):
        print("Computing KDE")
        ...
        KDE_cont_lys=consecutive("KDE_invert", 10, threshold_cont_KDE, deep_df)
    ...
    def calculate_intersections_wrapper(lys_x, lys_y, deep_df, threshold_cont_intersections):
        ...
        intersect_cont=consecutive("all_intersect", 10, threshold_cont_intersections, deep_df)
    ...
    def calculate_accuracy(sim_tracks2, finger_tracks, mean_msd_df):
        ...
        list_accuracy=[percent_both_confined, percent_both_unconfined, percent_correct, percent_correct_confined, percent_correct_unconfined,percent_sim_confined, percent_sim_unconfined,
                      precision_confined,  precision_unconfined,recall_confined, recall_unconfined,  fbeta_confined,fbeta_confined,fbeta_confined, support_confined, support_unconfined, logD_mean_diff, logD_mean_cluster_diff, mean_total_clusters_per_track, total_time_in_clusters, mean_time_per_cluster_per_track,mean_clustered_points, tpr, fpr]
